...
crtSentinel :: (Tensor t, Fact m, CRTrans Maybe (TRep t r), TElt t r)
...
crtCSentinel :: forall t m r .
                (Tensor t, Fact m, CRTrans Maybe (TRep t r), TElt t r)
                => Maybe (CSentinel t m r)
crtCSentinel = proxyT hasCRTFuncs (Proxy::Proxy (t m r)) *>
               pure CSentinel
{-# INLINABLE crtCSentinel #-}

crtESentinel :: (Tensor t, Fact m, CRTrans Maybe (TRep t r), TElt t r)
...
scalarCRTCS :: (Tensor t, Fact m, CRTrans Maybe (TRep t r), TElt t r)
...
embedCRTCS :: (Tensor t, m `Divides` m', CRTrans Maybe (TRep t r), TElt t r)
              => CSentinel t m r -> CSentinel t m' r -> t m r -> t m' r
embedCRTCS _ _ = fromJust embedCRT
{-# INLINABLE embedCRTCS #-}

twaceCRTCS :: (Tensor t, m `Divides` m', CRTrans Maybe (TRep t r), TElt t r)
              => CSentinel t m' r -> CSentinel t m r -> t m' r -> t m r
